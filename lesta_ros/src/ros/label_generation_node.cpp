 ...
 */
 ...
 LabelGenerationNode::LabelGenerationNode() : nh_("~") {
   std::cout << "== [DEBUG] Entering LabelGenerationNode constructor ==" << std::endl;
   ...
   // ROS node
   std::cout << "== [DEBUG] In loadConfig() ==" << std::endl;
 ...
 void LabelGenerationNode::loadConfig(const ros::NodeHandle &nh) {
   std::cout << "[DEBUG] Loading parameters..." << std::endl;
 
   if (!nh.getParam("lidarscan_topic", cfg_.lidarscan_topic)) {
     cfg_.lidarscan_topic = "/velodyne_tilted/velodyne_points";
     std::cout << "[WARNING] 'lidarscan_topic' not found. Using default: "
               << cfg_.lidarscan_topic << std::endl;
   } else {
     std::cout << "[DEBUG] lidarscan_topic = " << cfg_.lidarscan_topic << std::endl;
   }
 
   if (!nh.getParam("map_publish_rate", cfg_.map_pub_rate)) {
     cfg_.map_pub_rate = 10.0;
   }
   if (!nh.getParam("pose_update_rate", cfg_.pose_update_rate)) {
     cfg_.pose_update_rate = 10.0;
   }
   if (!nh.getParam("remove_backpoints", cfg_.remove_backpoints)) {
     cfg_.remove_backpoints = true;
   }
   if (!nh.getParam("debug_mode", cfg_.debug_mode)) {
 ...
 void LabelGenerationNode::initializePubSubs() {
   std::cout << "initializePubSubs Working.." << std::endl;
 ...
 void LabelGenerationNode::lidarScanCallback(const sensor_msgs::PointCloud2Ptr &msg) {
   std::cout << "== [DEBUG] lidarScanCallback called ==" << std::endl;
   geometry_msgs::TransformStamped sensor2base, base2map;
   bool tf1 = tf_.lookupTransform(frame_id_.robot, frame_id_.sensor, sensor2base);
   bool tf2 = tf_.lookupTransform(frame_id_.map, frame_id_.robot, base2map);
   ...
   if (!tf1 || !tf2) return;
     ...
     if (!tf_.lookupTransform(frame_id_.robot, frame_id_.sensor, sensor2base) ||
       ...
       return;
     std::cout << "[DEBUG] TF lookup: base→sensor = " << tf1 << ", map→base = " << tf2 << std::endl;
